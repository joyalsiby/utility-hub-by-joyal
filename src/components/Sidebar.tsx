...
import { Type, Home, Video, QrCode, Palette, Image as ImageIcon, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
const Sidebar = ({ isCollapsed, toggleSidebar }: SidebarProps) => {
  ...
  return (
    ...
    >
      <div className={`h-[70px] border-b border-slate-100 flex items-center transition-all duration-300 ${isCollapsed ? 'justify-center px-2' : 'px-6'}`}>
        <div className="flex items-center gap-3 overflow-hidden">
          <div className="bg-blue-600 p-2 rounded-lg flex-shrink-0">
            <Logo className="text-white w-5 h-5" />
          </div>
          <span className={`font-semibold text-slate-800 text-lg whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
            Utility Hub
          </span>
        </div>
      </div>
